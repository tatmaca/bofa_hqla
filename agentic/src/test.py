...
# ============================================================================
# Building auxilliary curves for greek calcs and credit products
# ============================================================================
...
for rating in recovery_rates.keys():
    for change, crv_dict in zip(changes, crv_dicts):
        ...
        crv_dict[rating] = ql.DefaultProbabilityTermStructureHandle(hazard_rate_curve)
...
sofr_term_structure = ql.FlatForward(
    today, ql.QuoteHandle(ql.SimpleQuote(sofr_rate)), day_count, ql.Continuous
)
...
lex_df, lex_result, lex_total, lex_return, lex_lcr = (
    optimizer.lexicographic_mean_optimize(
...
mv_df, mv_result, mv_total, mv_return, mv_lcr = optimizer.mean_variance_lexicographic(
    ...
    up_curve=base_curve_up,
    down_curve=base_curve_down,
...
print(
    f"4. Level 2A/2B assets have higher yields but haircuts limit their attractiveness"
)
print("=" * 80)
