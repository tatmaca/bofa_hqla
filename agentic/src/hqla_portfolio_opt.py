...
class HQLA_Portfolio_Opt_Enhanced:
    ...
    def _compute_hqla_value(self, weights: np.ndarray) -> float:
        ...
        level1_hqla = np.sum(
            market_values[df["Level"] == "L1"] * (1 - df.loc[df["Level"] == "L1", "Haircut"].values)
        )
        level2a_hqla = np.sum(
            market_values[df["Level"] == "L2A"] * (1 - df.loc[df["Level"] == "L2A", "Haircut"].values)
        )
        level2b_hqla = np.sum(
            market_values[df["Level"] == "L2B"] * (1 - df.loc[df["Level"] == "L2B", "Haircut"].values)
        )
    ...
    def _build_base_constraints(self, levels, max_total_allocation=2.0):
        ...
        constraints.append({
            "type": "ineq", 
            "fun": lambda w: max_total_allocation - np.sum(w)
        })
        
        # Basel III composition: Level 1 at least 60% of allocated weights
        constraints.append({
            "type": "ineq", 
            "fun": lambda w: np.sum(w[levels == "L1"]) - 0.6 * np.sum(w) if np.sum(w) > 1e-6 else 0.0
        })
        
        # Level 2B max 15% of allocated weights
        constraints.append({
            "type": "ineq", 
            "fun": lambda w: 0.15 * np.sum(w) - np.sum(w[levels == "L2B"]) if np.sum(w) > 1e-6 else 0.0
        })
        
        # LCR constraints
        constraints.append({
            "type": "ineq",
            "fun": lambda w: self._compute_hqla_value(w) / self.net_cash_outflow - self.min_lcr
        })
        constraints.append({
            "type": "ineq",
            "fun": lambda w: self.max_lcr - self._compute_hqla_value(w) / self.net_cash_outflow
        })
    ...
    def lexicographic_mean_optimize(
        ...
        min_lcr_achieved = self._compute_hqla_value(stage1_result.x) / self.net_cash_outflow
        
        if verbose:
            print(f"\nStage 1 Results:")
            print(f"  Minimum allocation: {min_allocation:.4f} × NCO = ${min_allocation * self.net_cash_outflow:,.2f}")
        ...
        stage2_constraints.append({
            "type": "ineq",
            "fun": lambda w: max_allowed_allocation - np.sum(w)
        })
        ...
        output_df = df[["Name", "Level", "DirtyPrice", "YTM", "ModDuration", "Allocated_Amount"]]
        ...
        if verbose:
            ...
            print(f"  Excess over minimum: {(total_allocated/self.net_cash_outflow - min_allocation):.2%}")
            print(f"Net Cash Outflow: ${self.net_cash_outflow:,.2f}")
            print(f"Expected Return: {expected_return:.4%}")
            print(f"  Improvement over Stage 1: {expected_return - np.dot(stage1_result.x, mu):.4%}")
            print(f"LCR Ratio: {lcr:.2%} (target: {self.min_lcr:.0%}-{self.max_lcr:.0%})")
    ...
    def mean_variance_lexicographic(
        ...
        if verbose:
            ...
            print(f"Total Allocated: ${total_allocated:,.2f} ({total_allocated/self.net_cash_outflow:.2%} of NCO)")
    ...
    def mean_variance_optimize_enhanced(
        ...
        levels = self.assets_summary["Level"].values
        constraints.append({"type": "ineq", "fun": lambda w: np.sum(w[levels == "L1"]) - 0.6})
        constraints.append({"type": "ineq", "fun": lambda w: 0.15 - np.sum(w[levels == "L2B"])})
        ...
        print(f"Expected Return: {port_return:.4%}")
        print(f"Portfolio Volatility: {port_std:.4%}")
        print(f"Sharpe Ratio (approx): {port_return/port_std if port_std > 0 else 0:.2f}")
        print(f"LCR Ratio: {lcr:.2%}")
        print(f"Portfolio Duration: {portfolio_duration:.2f} years")
...
class HQLA_Portfolio_Opt:
    ...
    def mean_optimize(self):
        ...
        df["Opt_Weight"] = result.x
        df["Weighted_Return"] = df["YTM"] * df["Opt_Weight"]
        ...
        # Compute total portfolio value and expected return and lcr
        total_value = df["Allocated_Amount"].sum()
        expected_return = float(np.dot(result.x, mu))
        lcr = self._lcr_adjusted_value(result.x) / self.net_cash_outflow
    ...
    def mean_variance_optimize(self, lam: float = 0.5,
        ...
        total_value = df["Allocated_Amount"].sum()
        expected_return = float(np.dot(res.x, mu))
        lcr = self._lcr_adjusted_value(res.x) / self.net_cash_outflow

        # Select only requested columns for output
        output_df = df[["Name", "Level", "Price", "YTM", "Allocated_Amount"]]
        
        return output_df, res, total_value, expected_return, lcr
