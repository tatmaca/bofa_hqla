...
class HQLA_Portfolio_Opt:
    ...
    def __init__(
        self, portfolio: HQLA.Portfolio, net_cash_outflow: float, max_lcr: float
    ):
        self.portfolio = portfolio
        self.assets = portfolio.assets
        self.net_cash_outflow = net_cash_outflow
    ...
    def mean_optimize(self):
        ...
        constraints = [
            ...
            {
                "type": "ineq",
                "fun": lambda w: self._lcr_adjusted_value(w) / self.net_cash_outflow
                - 1.0,
            },
            {
                "type": "ineq",
                "fun": lambda w: self.max_lcr
                - self._lcr_adjusted_value(w) / self.net_cash_outflow,
            },
        ]
    ...
    def mean_variance_optimize(
        ...
        # Compute allocated amount
        df["Allocated_Amount"] = np.round(
            df["Opt_Weight_MV"] * self.net_cash_outflow, 3
        )
